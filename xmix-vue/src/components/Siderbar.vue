<template>
  <div class="sidebar" :class="toCollapse">
    <span class="logo" v-show="isCollapse" @click="changeCollapse" :class="logo">XIN</span>
    <span class="logo-expand" v-show="!isCollapse" @click="changeCollapse">导航 </span>
    <div class="side-wrapper" :class="sideWrapper">
      <div class="side-title" :class="sideTitle">MENU</div>
      <div class="side-menu">
        <router-link class="sidebar-link discover" :class="{'is-active':$route.meta.path==='Blog'}" to="/Blog">
          <svg :class="isMargin" viewBox="0 0 24 24" fill="currentColor">
            <path d="M9.135 20.773v-3.057c0-.78.637-1.414 1.423-1.414h2.875c.377 0 .74.15 1.006.414.267.265.417.625.417 1v3.057c-.002.325.126.637.356.867.23.23.544.36.87.36h1.962a3.46 3.46 0 002.443-1 3.41 3.41 0 001.013-2.422V9.867c0-.735-.328-1.431-.895-1.902l-6.671-5.29a3.097 3.097 0 00-3.949.072L3.467 7.965A2.474 2.474 0 002.5 9.867v8.702C2.5 20.464 4.047 22 5.956 22h1.916c.68 0 1.231-.544 1.236-1.218l.027-.009z" />
          </svg>
          About Me
        </router-link>
        <router-link to="/StreamPage" class="sidebar-link trending" :class="{'is-active':$route.meta.path==='StreamPage'}">
          <svg :class="isMargin" viewBox="0 0 24 24" fill="currentColor">
    ...
    <div class="side-wrapper" :class="sideWrapper">
      <div class="side-title" :class="sideTitle">CATEGORY</div>
      <div class="side-menu">
        <a class="sidebar-link" href="#">
          <svg :class="isMargin" viewBox="0 0 24 24" fill="currentColor">
...
export default {
  ...
  data(){
    return{
      ...
      sideWrapper:{
        'side-wrapper-collapse':false
      },
      logoExpand:{
        'logo-expand-collapse':false,
      },
      sideTitle:{
        'side-title-collapse':false,
      },
      isCollapse: false,
      toCollapse:{
        collapse:false
  ...
  methods: {
    changeCollapse(){
        //TODO logo-expand  需要加个防抖
        this.isCollapse = !this.isCollapse

        this.toCollapse.collapse = !this.toCollapse.collapse
        //logo
        this.logo["logo-collapse"] = !this.logo["logo-collapse"]
        //side-wrapper
        this.sideWrapper["side-wrapper-collapse"] = !this.sideWrapper["side-wrapper-collapse"]
        //side-title
        this.sideTitle["side-title-collapse"] = !this.sideTitle["side-title-collapse"]
        //margin
        this.isMargin["is-Margin"] = !this.isMargin["is-Margin"]
...
.sidebar {
  ...
  flex-shrink: 0;  /*属性固定在相同的容器中，项目相对于其余弹性项目的收缩量,值越小宽度越大*/
  transition-duration: 0.2s; /*过渡的时间*/
  overflow-y: auto;   /*设置x轴内容溢出的内容的处理方式：此处为，若内容溢出则提供滚轮*/
  overflow-x: hidden;  /*设置x轴内容溢出的内容的处理方式：此处为隐藏*/
}
/*通过Js控制是否显示*/
.sidebar .logo{
  width: 30px;
...
.logo-expand {
  ...
  font-weight: 600;
  line-height: 34px;
  position: sticky;   /*将元素的定位类型设置为sticky，该元素在滚动到一定位置时会固定在屏幕上*/
  top: 0;             /*设置上面滚动的距离，和sticky一起使用：此处表示该元素滚动到距离顶部0px时会被固定住*/
}
.logo-expand:before{
  content: "";
  position: absolute;
  top: -30px;
  left: 0;    /*以上俩行用于设置元素的左上角的位置*/
  background: #1f1d2b;
  width: 200px;
  height: 70px;
  z-index: -1;  /*在当前元素logo-expand的下方*/
}
.sidebar-link:hover,
...
@keyframes bottom {   /*定义动画*/
  0% {
    transform: translateY(100px);   /*初始状态*/
    opacity: 0;
  }
...
/*只有当通过Js添加了collapse这个选择器的时候，才加上下面的样式*/
.is-Margin {
